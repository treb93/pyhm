    ...
    def forward(self,
        ...
        if self._aggre_type == 'mean':
            ...
            if graph.canonical_etypes[0][0] in [
                    'user',
                    'item'] and graph.canonical_etypes[0][2] in [
                    'user',
                    'item']:
    ...
    def __init__(self,
        ...
        if embedding_layer:
            self.user_embed = NodeEmbedding(
                dim_dict['user'], dim_dict['hidden'])
            self.item_embed = NodeEmbedding(
                dim_dict['item'], dim_dict['hidden'])
            if 'sport' in g.ntypes:
                self.sport_embed = NodeEmbedding(
                    dim_dict['sport'], dim_dict['hidden'])
...
def max_margin_loss(pos_score,
    ...
    for etype in pos_score.keys():
        ...
        if remove_false_negative:
            negative_mask_tensor = negative_mask[etype].reshape(
                -1, neg_sample_size)
        else:
        ...
        if use_recency:
            ...
            # Not all edge types have recency. Only training edges have recency
            # (i.e. clicks & buys)
            except KeyError:
                pass
